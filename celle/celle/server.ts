...
async function getCollectionDocs(collectionName: string): Promise<any[]> {
  ...
  try {
    ...
    console.warn(`[Firestore Status] Firestore collection '${collectionName}' is temporarily inaccessible on 'celle-e7ecb' (${error instanceof Error ? error.message : "Access restricted"}). Serving from local offline cache.`);
    const local = readDb();
    return local[collectionName] || [];
  }
}
...
async function addCollectionDoc(collectionName: string, id: string, data: any): Promise<void> {
  ...
  try {
    ...
    console.warn(`[Firestore Status] Direct cloud write to collection '${collectionName}' failed (${error instanceof Error ? error.message : "Access restricted"}). Safely persisted in fallback queue.`);
  }
}
...
async function updateCollectionDoc(collectionName: string, id: string, updatedFields: any): Promise<void> {
  ...
  try {
    ...
    console.warn(`[Firestore Status] Direct cloud update to collection '${collectionName}' failed (${error instanceof Error ? error.message : "Access restricted"}). Safely updated local model.`);
  }
}
...
async function syncDatabaseToFirestore() {
  ...
  try {
    ...
    console.warn(`[Firestore Setup] Synchronized seed check skipped (${error instanceof Error ? error.message : "Access restricted"}). App will operate using native local cache.`);
  }
}
